...
class SurgicalDINOEvidential(nn.Module):
    def __init__(
        ...
        dinov2 = torch.hub.load(
            repo_or_dir="facebookresearch/dinov2", model=f"dinov2_{self.backbone_arch}"
        )

        if lora_layer:
            self.lora_layer = lora_layer
        else:
            self.lora_layer = list(range(len(dinov2.blocks)))
        ...
        if decode_type == 'linear4':
            self.inchannels = [self.embedding_dim] * 4
    ...
    def get_uncertainty(self, nig_params):
    ...
    def save_parameters(self, filename: str) -> None:
        ...
        if isinstance(
            self.decode_head, (nn.DataParallel, nn.parallel.DistributedDataParallel)
        ):
